...
const getTime = (format, date) => {
	if (date) {
		...
		return moment.tz('Asia/Karachi').locale('en-PK').format(format)
	}
}

const formatDate = (n, locale = 'en-PK') => {
	let d = new Date(n)
...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./ERROR-V1Media/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./ERROR-V1Media/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./ERROR-V1Media/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./ERROR-V1Media/trash/${filename}.mp4`)
    Promise.all([unlink(`./ERROR-V1Media/trash/${filename}.mp4`), unlink(`./ERROR-V1Media/trash/${filename}.gif`)])
    return buffer5
}
