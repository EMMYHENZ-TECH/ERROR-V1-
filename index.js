...
(async () => {
	...
	if (loadData && Object.keys(loadData).length === 0) {
		...
		await database.write(ERROR-V1.db)

	} else {

		ERROR-V1.db = loadData
	...
	setInterval(async () => {
		...
		if (ERROR-V1.db) await database.write(ERROR-V1.db)
		...
		if (db.set[botNumber].anticall) {
			...
			for (let id of call) {
				...
				if (id.status === 'offer') {
					...
					await XeonBotInc.sendContact(id.from, ERROR-V1.owner, msg);
